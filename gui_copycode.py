...
# <<< NEW: Import the themed widget library (ttk) and font module
...
class CodeCopierApp:
    ...
    # <<< NEW: A dictionary to define our app's color palette for easy changes.
    COLORS = {
        "bg": "#F0F0F0",          # Light gray background
        "bg_list": "#FFFFFF",     # White background for the list area
        "text": "#1F1F1F",        # Dark text
        "accent": "#0078D7",      # A nice blue for accents
        "accent_fg": "#FFFFFF",   # White text on the accent color
        "green": "#107C10",       # Green for directory names
        "blue": "#00539C"         # Blue for "Go Up" link
    }

    def __init__(self, root):
        self.root = root
        self.root.title("Code Selector & Copier")
        self.root.geometry("650x550") # A little more space
        self.root.minsize(500, 400)
        self.root.configure(bg=self.COLORS["bg"]) # Set main background color

        # <<< NEW: Configure the style engine for our themed widgets
        ...
        self.style.configure('Status.TLabel', font=('Segoe UI', 8)) # Smaller font for status bar
        self.style.configure('Header.TLabel', font=('Segoe UI', 10, 'bold')) # Bigger font for path
        
        # Style for the main "Copy" button
        self.style.configure('Accent.TButton', background=self.COLORS["accent"], foreground=self.COLORS["accent_fg"])
        self.style.map('Accent.TButton', background=[('active', '#005A9E')]) # Darker blue on hover

        # Style for clickable directory labels
        ...
        # --- UI FRAMES ---
        # <<< CHANGED: Using ttk.Frame and adding more padding (pady)
        ...
        # Using a tk.Frame here for the sunken border, as ttk.Frame doesn't support it directly
        ...
        # --- PATH & HIDE WIDGETS ---
        # <<< CHANGED: Using ttk widgets with our custom styles
        ...
        self.scrollable_frame = ttk.Frame(self.canvas, style='TFrame') # Use our styled frame
        self.scrollable_frame.configure(style='List.TFrame') # Use a dedicated style for the list background
    ...
    def load_files(self):
        ...
        try:
            ...
            # <<< CHANGED: Using styled ttk.Labels for directory navigation
            ...
            for f in files:
                ...
                # <<< CHANGED: Using styled ttk.Checkbutton
                cb = ttk.Checkbutton(self.scrollable_frame, text=f, variable=var, style='TCheckbutton',
                                          command=lambda p=full_path, v=var: self.toggle_selection(p, v))
                cb.configure(style='List.TCheckbutton') # Apply list-specific style
    ...
    def select_all_displayed_files(self):
        ...
        # Re-sync the main set after batch operation
